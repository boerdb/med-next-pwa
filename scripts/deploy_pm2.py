...
run(ssh, "pm2 delete med-track-pwa 2>/dev/null; pm2 delete med-next-pwa 2>/dev/null; true", check=False)
run(ssh, f"cd {REMOTE_DIR} && pm2 start npm --name med-track-pwa -- start")
run(ssh, "pm2 save")
